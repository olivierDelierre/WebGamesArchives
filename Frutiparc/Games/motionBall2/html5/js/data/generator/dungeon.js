/*
 * Random dungeon map. Port of mb2gen/dungeon.ml.
 *
 * Builds an 8 x 8 grid of rooms connected by exits :
 *   1. carve a random tree of rooms from the start (genRoom),
 *      pick a random exit room, add random extra openings / walls (genPath),
 *      keep only the rooms reachable from the start ;
 *   2. put the 4 balls (green, blue, metal, violet) in dead ends, and the
 *      rooms / doors that need them along the way (genObjects) ;
 *   3. check that the exit is far enough and needs the 4 balls (checkDifficulty) ;
 *   4. put bonuses (map, radar, keys, time...) in the remaining dead ends,
 *      sometimes behind invisible exits (genObjectsFinal).
 * Any failed check throws Gen.Retry and the whole map is generated again.
 *
 * The map uses its own structure (rooms with rup / rdown / rleft / rright
 * paths and a typed rtype), converted to the game format in challenge.js.
 */
"use strict";
...
(function () {
	...
	const { Retry, rnd, rbool, shuffle, newTable } = Gen;
	...
	Gen.Dungeon = {
		...
		generate(w, h) {
			return Gen.retry(Infinity, () => genDungeonOnce(w, h));
		},

		/** An empty map of normal rooms without exits (Classique mode). */
		empty(w, h) {
			const d = newDungeon(w, h);
			d.dstart = { x: 0, y: 0 };
			d.dexit = { x: 0, y: 0 };
