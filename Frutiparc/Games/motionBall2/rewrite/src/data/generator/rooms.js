/**
 * The random content of a room. Follows the room part of mb2gen/level.ml.
 *
 * A room is generated on a table of its 152 x 102 cells (4 x 4 pixels),
 * where each cell holds what occupies it : border, bumper, pastille, green
 * block or hole.
 *
 * The generator places, in order :
 *   - the doors (clearing the border), and the line of blocks / holes of the
 *     doors that need a ball,
 *   - the Classique hatch,
 *   - the ring of blocks / holes around a pastille of the rooms that need a
 *     ball (genObjectNeed),
 *   - up to two random walls across the room (genSeparators),
 *   - the bumpers, more and more dangerous with the room level ;
 * then it computes which cells the ball can reach from the entrance
 * (computeRoomTbl), to put the pastilles only where they can be collected,
 * and to check that every exit can be reached.
 */

import { Item as B, DungeonBall as Obj } from "../enums.js";
import { Retry, rnd, rbool, random2, newTable, retry, Layout, Shape } from "./util.js";
...
const bumperForObject = o => (o === Obj.GREEN) ? B.BLOCK : (o === Obj.BLUE) ? B.HOLE : (o === Obj.METAL) ? B.MAGNET : B.GHOST;
...
function genNormalRoom(ctx, r, lvl, obj) {
	...
	if (!classic) {
		...
		blist.unshift([B.HATCH, classic.exit]);
	}
	...
	if (!classic)
		blist = blist.concat(genSeparators(r, ctbl, rnd(3) === 0 ? B.HOLE : B.BLOCK, Math.max(5 - (lvl / 4 | 0), 1)));
	...
	for (let i = 0; i < nbumpers; i++) {
		if (rnd(100) < ((lvl * 100 / 40) | 0)) {
			if (availableMagnet > 0) {
				availableMagnet--;
				bumpers.push([B.MAGNET, L.shapes[Shape.MAGNET]]);
			} else if (availableShadow > 0) {
				availableShadow--;
				bumpers.push([B.GHOST, L.shapes[Shape.GHOST]]);
			} else if (availableBtime > 0 && rnd(5) !== 0) {
				availableBtime--;
				bumpers.push([B.CLOCK, L.shapes[Shape.CLOCK]]);
			} else {
				bumpers.push([B.DEATH, L.shapes[Shape.DEATH]]);
			}
		} else {
			bumpers.push([B.BUMPER, L.shapes[Shape.BUMPER]]);
		}
	}

	// place them (magnets and death bumpers away from the doors)
	for (const [btype, btbl] of bumpers) {
		// (the original looped forever when only places near the border were left)
		let pos;
...
function genSpecialRoom(r, obj) {
	const ctbl = newColTable();
	const btype = (obj === Obj.METAL) ? B.MAGNET : B.GHOST;
	const btbl = (obj === Obj.METAL) ? L.shapes[Shape.MAGNET] : L.shapes[Shape.GHOST];
 ...
 *   tries = attempts before giving up (throws Retry)
 * Returns a list of [Item, { x, y }], or null for the rooms without items.
 */
export function generateRoom(ctx, r, lvl, tries = 1000) {
	return retry(tries, () => genRoomOnce(ctx, r, lvl));
}

/** A random position for the hatch of a Classique room. */
export function randomHatch() {
	return {
